...
class RFClassifier():
    ...
    def data_selection(self, training):
        ...
        """
        Hardcoding the chosen 9 attributes and then the 7 attributes (without the two best ones for r&b according to sci kit learn).
        Random attributes (m=4 in our case) are then chosen randomally. 
        """
        attributes_ = ['danceability', 'energy', 'loudness','speechiness', 'acousticness', 'instrumentalness', 
                          'liveness', 'valence', 'tempo']
        attributes_without_best = ['danceability', 'loudness','speechiness', 'acousticness', 'instrumentalness', 
                          'liveness', 'valence']
        
        attributes_without_worst = ['danceability', 'energy', 'loudness','speechiness', 'acousticness', 'valence', 'tempo']
        
        cols = random.sample(attributes_without_worst, self.m) # without replacement
        cols.append(list(training.columns)[-1]) #last column is class label
    ...
    def random_forest(self, training):
        ...
        for i in range(self.N):
            ...
            tree = classifier.C45(tree_data, list(tree_data.columns)[:-1], og_tree_data, threshold=0.001) # credit
            forest.append(tree)
        return forest
    ...
    def cross_validation(self):
        ...
        n = 10 #lowered to 5 fold cross-val for some now back at 10
        ...
        file_name = dataset + '_rf_balanced_wo_worst5.txt' #now running with 500 datapoints and 10-fold cross val
...
if __name__ == '__main__':
    ...
    D['new_target'] = np.where(D['playlist_genre'] == 'r&b', 1, 0)

    ##BALANCING the dataset
    #UNDERSAMPLING to increase the percentage of r&b (currently 15%) to 50%
    #Extracting all 1s
    df_ones = D[D['new_target'] == 1]

    #subsampling the 0s (to decrease the number of them)
    df_zeros = D[D['new_target'] == 0]
    df_zeros = df_zeros.sample(n=5429)

    #concatenate
    dfs = [df_ones, df_zeros]
    D = pd.concat(dfs) #updating D, 10860 rows now (balanced between r&b and non-r&b)
...
# python3 randomForest.py iris.csv 2 75 75
# python3 randomForest.py credit.csv 7 350 50
# python3 randomForest.py heart.csv 6 460 75