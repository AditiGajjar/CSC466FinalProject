...
class DecisionTreeClassifier():
    ...
    def selectSplittingAttribute(self, D, A, threshold): # uses information gain
        ...
        if gains[best] > threshold and len(D[best].unique()) > 1:
            return best
        else:
            return None
...
if __name__ == '__main__':
    ...
    T = classifier.C45(D, A, og_D, threshold=2) # spotify
    T = {'dataset': csv_file, 'node': T}
    with open(dataset + '.json', 'w') as outfile:
        json.dump(T, outfile, indent=4)

# python3 InduceC45.py iris.csv iris.txt
# python3 InduceC45.py heart.csv heart.txt
# python3 InduceC45.py credit.csv credit.txt